...
TARGETS_DB="${CONFIG_DIR}/targets.tsv"#!/usr/bin/env bash
...
tcp_check() {
    local host="$1"
    local port="$2"
    local output count interval_us total_timeout
    TCP_CHECK_ERROR=""
    ...
    interval_us="${TCP_CHECK_INTERVAL_US:-200000}"
    [[ "${interval_us}" =~ ^[0-9]+$ && "${interval_us}" -ge 1 ]] || interval_us=200000
    total_timeout="${TCP_CHECK_TOTAL_TIMEOUT:-}"
    [[ "${total_timeout}" =~ ^[0-9]+$ && "${total_timeout}" -ge 1 ]] || total_timeout="$((CHECK_TIMEOUT + 3))"

    output="$(timeout "${total_timeout}" hping3 -S -p "${port}" -c "${count}" -i "u${interval_us}" "${host}" 2>&1)"
...
update_self() {
    ...
    if [[ "${target_path}" != "${script_path}" && "${1:-}" != "--no-exec" ]]; then
